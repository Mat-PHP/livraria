...
import { ImagemService } from '../../services/imagens.services';
...
export class ImagemComponent {
  ...
  carregar() {
    this.svc.listar().subscribe({
      next: (data: any) => {
        ...
        this.imagem.set(Array.isArray(data) ? data : data.results ?? []);
      },
  ...
  onSubmit(ev: Event) {
    ...
    this.svc.enviar(this.arquivo).subscribe({
      next: (img) => {
        ...
        this.arquivo = null;
        this.preview = null;
        this.imagem.update((arr) => [img, ...arr]);
      },
      error: (err) => {
        console.error(err);
  ...
  remover(id: number) {
    this.svc.deletar(id).subscribe({
      next: () => this.imagem.update((arr) => arr.filter(i => i.id !== id)),
